...
public class AccessibleStepConfigReader {
	...
	public static AccessibleStepConfig readConfig(MinecraftClient client) {
		...
		if (!configFile.exists()) {
			AccessibleStep.LOGGER.info(String.format("Creating config file for %s.", AccessibleStep.MOD_ID));
		...
		try {
			...
			AccessibleStep.LOGGER.error(String.format("Unable to read config file. Revering to default settings."));
	...
	public static void writeConfig(AccessibleStepConfig config) {
		...
		if (!configFile.exists()) {
			try {
				...
				AccessibleStep.LOGGER.error(String.format("Unable to create config file. Pending changes will be lost."));
				return;
			}
		}
		...
		try {
			...
			AccessibleStep.LOGGER.error(String.format("Unable to save config file. Pending changes will be lost."));
		}
	}
}
